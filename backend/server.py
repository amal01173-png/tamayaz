...
async def register(user_data: UserCreate):
    ...
    if user_data.role == "student" and not user_data.email:
        # Create unique email from name, class, and UUID to avoid duplicates
        safe_name = user_data.name.replace(" ", "_").lower()
        safe_class = user_data.class_name.replace("/", "_") if user_data.class_name else "noclass"
        # Add UUID to ensure uniqueness even with duplicate names
        unique_id = str(uuid.uuid4())[:8]
        user_data.email = f"{safe_name}_{safe_class}_{unique_id}@tamayyuz.local"
    
    # For non-students, check if email exists
    if user_data.role != "student" and user_data.email:
...
async def create_student(student_data: StudentCreate, current_user: dict = Depends(get_current_user)):
    ...
    if not user_id:
        # Create unique email with UUID to avoid duplicates
